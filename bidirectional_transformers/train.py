...
from config import get_config, get_weights_file_path, latest_weights_file_path, current_directory
...
#         trainer = WordLevelTrainer(specical_tokens=["[UNK]", "[PAD]", "[SOS]", "[EOS]"],
#                                    min_frequency=2)
...
def get_or_build_tokenizer(config, ds, lang):
    tokenizer_path = Path(current_directory + '/' +config['tokenizer_file'].format(lang))
    if not Path.exists(tokenizer_path):
...
def get_ds(config):
    ...
    # build tokenizers 
    tokenizer_src = get_or_build_tokenizer(config, 
                                           ds_raw, 
                                           config['lang_src'])
    tokenizer_tgt = get_or_build_tokenizer(config,
                                           ds_raw,
                                           config['lang_tgt'])
    ...
    # Find the maximum length of each sentence in the source & target sentences 
    max_len_src = 0
    max_len_tgt = 0
...
def train_model(config):
    ...
    weights_folder_path = f"{current_directory}/{config['datasource']}_{config['model_folder']}"
    print(f'weights_folder_path: {weights_folder_path}')
    Path(weights_folder_path).mkdir(parents=True, exist_ok=True)
